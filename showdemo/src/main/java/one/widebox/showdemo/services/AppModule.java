...
public class AppModule {
	...
	public static void bind(ServiceBinder binder) {
		binder.bind(AppService.class);
		binder.bind(PasswordService.class);
		binder.bind(SelectModelService.class);
		binder.bind(StudentService.class);
		binder.bind(UserService.class);
	}

	// public static void contributeHibernateEntityPackageManager(
	// Configuration<String> configuration) {
	// configuration.add("one.widebox.showdemo.entities");
	// }
	...
	public static void contributeApplicationDefaults(MappedConfiguration<String, Object> configuration) {
		configuration.add(FoggylandSymbols.OSS_ROOT_FOLDER, "C:/opt/oss/showdemo/");
		configuration.add(FoggylandSymbols.OSS_BUCKET_FORMAT, "yyyy/MM/dd");

		configuration.add(SymbolConstants.SUPPORTED_LOCALES, "zh_TW");
		configuration.add(HibernateSymbols.EARLY_START_UP, "true");
		configuration.add(SymbolConstants.HMAC_PASSPHRASE, "showdemo2019");
