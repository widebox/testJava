 ...
 * of several sub-components. The sub-components are statically wired to the
 * Grid, as it provides access to the data and other models that they need.
 *
 * A Grid may operate inside a
 * {@link org.apache.tapestry5.corelib.components.Form}. By overriding the cell
 * renderers of properties, the default output-only behavior can be changed to
 * produce a complex form with individual control for editing properties of each
 * row. There is a big caveat here: if the order of rows provided by the
 * {@link org.apache.tapestry5.grid.GridDataSource} changes between render and
 * form submission, then there's the possibility that data will be applied to
...
public class MyGrid implements GridModel, ClientElement {
	...
	@Parameter
	@Property(write = false)
	private Object[] addPageContext;

	@Parameter(value = BindingConstants.MESSAGE + ":add", defaultPrefix = BindingConstants.LITERAL)
	@Property(write = false)
	private String addLinkText;
	...
	public Link getLink() {
		if (addPageContext == null) {
			return linkSource.createPageRenderLinkWithContext(addPage);
		}
		return linkSource.createPageRenderLinkWithContext(addPage,
				addPageContext);
	}
	...
	@Parameter
	private Block gridOperationBlock;

	public Object getGridOperationBlock() {
		return gridOperationBlock;
	}
	...
	/**
	 * A wrapper around the provided GridDataSource that caches access to the
	...
	@Parameter(value = BindingConstants.SYMBOL + ":"
			+ ComponentParameterConstants.GRID_PAGER_POSITION, defaultPrefix = BindingConstants.LITERAL)
	private GridPagerPosition pagerPosition;

	/**
	 * Used to store the current object being rendered (for the current row).
	 * This is used when parameter blocks are provided to override the default
	 * cell renderer for a particular column ... the components within the block
	 * can use the property bound to the row parameter to know what they should
	 * render.
	 */
	...
	/**
	 * Optional output parameter used to identify the index of the column being
	 * rendered.
	 */
	@Parameter
	...
	/**
	 * CSS class for the &lt;table&gt; element. In addition, informal parameters
	 * to the Grid are rendered in the table element.
	 */
	...
	/**
	 * If true, then the Grid will also render a table element complete with
	 * headers if the data source is empty. If set to true, a model parameter
	 * will have to be specified. A default model for a specific class can be
	 * created using
	 * {@link BeanModelSource#createDisplayModel(Class, org.apache.tapestry5.ioc.Messages)}
	 * .
	 */
	@Parameter
	...
	/**
	 * The name of the pseudo-zone that encloses the Grid. Starting in 5.4, this
	 * is always either null or "^" and is not really used the way it was in
	 * 5.3; instead it triggers the addition of a
	 * {@code data-inplace-grid-links} attribute in a div surrounding any links
	 * related to sorting or pagination. The rest is sorted out on the client.
	 * See module {@code t5/core/zone}.
	 */
	@Property(write = false)
	private String zone;
	...
	/**
	 * The pagination model for the Grid, which encapsulates current page, sort
	 * column id, and sort ascending/descending. If not bound, a persistent
	 * property of the Grid is used. When rendering the Grid in a loop, this
	 * should be bound in some way to keep successive instances of the Grid
	...
	@Component(parameters = { "columnIndex=inherit:columnIndex",
	...
	class DefaultGridSortModel implements GridSortModel {
		public ColumnSort getColumnSort(String columnId) {
			if (paginationModel == null
					|| !TapestryInternalUtils.isEqual(columnId,
		...
		public List<SortConstraint> getSortConstraints() {
			...
			if (paginationModel == null
					|| paginationModel.getSortColumnId() == null) {
				return Collections.emptyList();
			}

			PropertyModel sortModel = getDataModel().getById(
					paginationModel.getSortColumnId());
	...
	static final ComponentAction<MyGrid> SETUP_DATA_SOURCE = new ComponentAction<MyGrid>() {
		private static final long serialVersionUID = 1L;

		public void execute(MyGrid component) {
	...
	Object setupRender() {
		...
		return !renderTableIfEmpty && cachingSource.getAvailableRows() == 0 ? empty
				: null;
	}
	...
	void cleanupRender() {
		...
		// If grid is rendered inside a Loop. be sure to generate a new data
		// model for
	...
	void setupDataSource() {
		...
		// This captures when the number of rows has decreased, typically due to
		// deletions.
		...
		int endIndex = Math
				.min(startIndex + rowsPerPage - 1, availableRows - 1);
	...
	Object beginRender(MarkupWriter writer) {
		...
		if (inPlace && zone == null) {
			...
			// Through Tapestry 5.3, we had a specific id for the zone that had
			// to be passed down to the
			// GridPager and etc. That's no longer necessary, so zone will
			// always be null or "^". We don't
	 ...
	 * and {@linkplain org.apache.tapestry5.grid.GridSortModel#clear() clears
	 * the sort model}.
	 */
	public void reset() {
		sortModel.clear();
